...
if not postiz_key:
    raise RuntimeError("The decrypted Postiz credential was empty")
environment = os.environ.copy()
environment["POSTIZ_KEY"] = postiz_key
environment["POSTIZ_API_URL"] = config.get("apiUrl", "https://api.postiz.com/public/v1")
try:
    completed = subprocess.run(["node", "src/repair-postiz-covers-cli.mjs"], env=environment, check=False)
finally:
